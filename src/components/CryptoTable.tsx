...
export function CryptoTable({ coins, onCoinClick }: CryptoTableProps) {
  ...
  return (
    <div className="bg-[#1a1a2e] border border-[#2a2a45] rounded-xl overflow-hidden">
      <div className="hidden md:grid grid-cols-[60px_1fr_120px_90px_90px_120px_120px] gap-4 p-4 border-b border-[#2a2a45] bg-[#252540]/30 text-sm font-medium text-[#a0a0b8]">
        <span>#</span>
        <span>Name</span>
...
function CryptoRowMobile({ coin, onCoinClick }: { coin: CryptoAsset; onCoinClick?: (coinId: number, coinName: string) => void }) {
  ...
  return (
    <div
      className="p-4 border-b border-[#2a2a45] hover:bg-[#252540]/50 transition-colors cursor-pointer"
    ...
    >
      <div className="flex items-center gap-3 mb-3">
        ...
        <div className="ml-auto text-right">
          <div className="font-mono text-white">{coin.price < 1 ? `$${coin.price.toFixed(6)}` : `$${coin.price.toFixed(2)}`}</div>
          <div className={`text-sm font-mono ${isPositive24h ? 'text-[#00d4aa]' : 'text-[#ff6b6b]'}`}>
